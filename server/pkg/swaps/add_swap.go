...
func (h handler) AddSwap(c *gin.Context) {
	...
	if batteryInSoc > batteryOutSoc { // if batteryInSoc is greater than batteryOutSoc
		usedSoc := batteryInSoc - batteryOutSoc // the used energy in terms of soc

		// Energy Consumed (kWh) = Used SOC (%) * Battery Capacity (kWh)
		energyConsumed = usedSoc / 100 * batteryIn.BatteryCapacity // the used energy in terms of kWh

		// Distance Covered (km) = Energy Consumed (kWh) / Consumption Rate (kWh/km)
		distanceCovered = energyConsumed / batteryIn.ConsumptionRate

		// Cost (RWF) = Energy Consumed (kWh) * Cost per kWh (Rs/kWh)
		cost = energyConsumed * COST_PER_KWH
	...
	swap.Cost = cost
	swap.DistanceCovered = distanceCovered
	swap.EnergyConsumed = energyConsumed
